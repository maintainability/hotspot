...
public class SourceControlLogic {
	public CommitFileMatrix readCommitData(
		...
		for (CommitData commitData : commitDataListFiltered) {
			...
			if (!context.isSaveLogsOnly()) {
				List<CommitFileCell> cumulativeCommitFiles = calculateCumulativeData(result, commitData, commitedFiles);
	...
	protected List<CommitFileCell> calculateCumulativeData(CommitFileMatrix commitFileMatrix, CommitData commitData, List<FileDiffInformation> commitedFiles) {
